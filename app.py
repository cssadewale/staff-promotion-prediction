...
# ─── Build Model (train from scratch on the server) ───────────────────────────
# Why retrain instead of loading a .joblib file?
# Streamlit Cloud runs Python 3.14 and ignores runtime.txt / .python-version.
# A model saved in Python 3.12 (Colab) cannot be unpickled in Python 3.14
# (Streamlit) — this causes an AttributeError. The solution is to retrain
# the model fresh on the server using the original dataset. Training takes
# ~60 seconds on first load, then is cached permanently for all future visits.
...
def build_model():
    ...
    X = df.drop(columns=["promoted_or_not"])
    ...
    # ── Step 8: Define feature groups ────────────────────────────────────────
    num_features = [
        "trainings_attended", "last_performance_score", "year_of_recruitment",
    ...
    # ── Step 11: Cache the trained model to disk ──────────────────────────────
    joblib.dump(full_pipeline, MODEL_CACHE)
...
# ─── Model Build Error Handling ───────────────────────────────────────────────
if model is None:
    st.error(
        "⚠️ **Model could not be built.**\n\n"
        "The app failed to download the training data from Google Drive. "
